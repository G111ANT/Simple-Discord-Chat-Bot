...
if __name__ == "__main__":
    ...
    async def on_message(message: discord.Message):
        ...
        pers: dict | None = pers_raw[0] if isinstance(pers_raw, tuple) and len(pers_raw) > 0 else None  # type: ignore
        ...
        if message.author.id != discord_client.application_id and not message.author.bot:
            ...
            if discord_client.application_id in map(lambda x: x.id, message.mentions):  # type: ignore
                ...
                if not respond and (datetime.timedelta(minutes=5) <= time_since_last_chat <= datetime.timedelta(minutes=10)):
        ...
        asyncio.create_task(message.add_reaction(random.choice(list("🌑🌒🌓🌔🌕🌖🌗🌘"))))
        ...
        logger.info(f'Sent "{(await chat.message_history_to_xml(message_history))[:100]}..." (newest) to the AI from history of {len(message_history)}')
