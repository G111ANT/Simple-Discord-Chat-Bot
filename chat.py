...
async def get_CoT(
    ...
    base_responses = [
        await AsyncClient(
            ...
            messages=await add_to_system(GLOBAL_SYSTEM, "", " Keep the message to a paragraph.") + messages,  # type: ignore
    ...
    for completions in range(len(base_content_filtered)):
        critique_prompt += f"\nCandidate {completions + 1}:\n{await tools.model_text_replace(base_content_filtered[completions], os.environ['SIMPLE_CHAT_THINK_MODEL_REPLACE'])}\n"

    critique_prompt += "\nPlease provide your critique for each candidate here:"

    critique_response = await AsyncClient(
        api_key=os.environ["SIMPLE_CHAT_OPENAI_KEY"],
        base_url=os.environ["SIMPLE_CHAT_OPENAI_BASE_URL"],
    ).chat.completions.create(
        messages=GLOBAL_SYSTEM
    ...
    return await tools.model_text_replace(
        final_content, os.environ["SIMPLE_CHAT_CHAT_MODEL_REPLACE"]
    )
